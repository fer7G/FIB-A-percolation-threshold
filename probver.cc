...
vector <float> calcularprobvertex(Graph a){
...
int percolaciovertex(vector<int> ProbVertex, Graph a, float p){
        ...
        if(nodosrestantes[i]){
            ...
            for(int j=0; j<n2 ; ++j){
                ...
                if( nodosrestantes[conexion]) {
                    inducido.add_edge(nodosfinales[i],nodosfinales[conexion] );
